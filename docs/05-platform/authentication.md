...
- **Ports served — `AccountLifecyclePort`** (added 2026-08-02, consumer employee.md): `createUserForEmployee(employeeId, email)` — creates the tenant user, links `employees.user_id`, assigns the Employee role template (authorization-rbac default duty), and queues `auth.invite`, all inside the caller's transaction (BR-EMP-002 hire); `deactivateUser(userId, reason)` — sets `users.status = 'inactive'` and revokes live sessions in the caller's transaction (employee.md BR-EMP-006 exit; login/refresh die immediately per BR-AUTH-002/§9, access tokens age out ≤ 15 min).
  ...
  **Built 2026-08-07 with its only consumer, and the first clause is split** (A-195): *linking `employees.user_id`* happens in the employee module rather than here, because `employees` is that module's table and ADR-0001 rule 5 gives writes to the owner alone. Same transaction, same outcome, and the write lands on the right side of the boundary. *Assigning the Employee role template* is **not built**: authorization-rbac seeds the ten templates and has not shipped, so there is no template to assign — and the consequence is benign, since every employee self-service route is `@AuthenticatedOnly()` (BR-AUTHZ-009) and an invited employee reaches the whole employee surface with no role at all.
- **Read-model views consumed — `employee_directory`** (employee.md §13; ADR-0001 rule 6 constraint (d) requires the declaration). `GET /api/v1/auth/me` has promised `user.name` and `user.employeeId` since §7 was written, and both live on `employees`; the view is the sanctioned channel — no encrypted column, no masked column, `security_invoker = true`, so the read runs under the caller's RLS. It creates no module import and therefore no cycle with a business module. `null` for a user with no employee row, which is what makes `employeeId` optional.
