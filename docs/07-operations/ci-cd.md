...
- **Squash merge only** in the three implementation repositories; rebase and merge-commit are disabled. Linear history, one commit per pull request. *(Scoped 2026-08-05: this clause read as universal once `ADR-0025` §2 extended branch protection to `hris-handbook`, which it never governed. The handbook repository is the stated exception below.)*
...
**`hris-handbook` is the exception, and it is the mirror image** *(added 2026-08-05, when protection was first applied)*. It runs gitflow — `develop` integrates, `release/*` merges to `main` with `--no-ff` and an annotated tag — so **merge commits are enabled there and squash and rebase are disabled**. Squash merge would delete the branch topology gitflow's release and back-merge steps exist to record, and a handbook release is a ratification event worth seeing in the graph rather than one commit per pull request. `ADR-0025` §2's protection is applied on both branches: pull request required on `main`, force-push and deletion blocked on `main` and `develop`, `enforce_admins` on, **zero** required approvals, and no required status checks. The last two are forced rather than chosen — one collaborator cannot approve their own pull request, so a `1` here deadlocks the repository, and a required check that no workflow reports blocks every pull request forever. Wiring `erd-check` and `guide-check` into CI is what makes that third setting safe to raise. A-180.
