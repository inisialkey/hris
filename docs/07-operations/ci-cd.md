...
**`hris-handbook` follows this section too** *(added 2026-08-05, when protection was first applied)*. `ADR-0019` §1's trunk-based model and naming-conventions §12's branch types are not scoped to the three implementation repositories, and the handbook repository was briefly and wrongly put on gitflow before that was noticed (issue #1). It is squash-merged onto a single `main` like everything else. Its protection: pull request required on `main`, force-push and deletion blocked, linear history required, `enforce_admins` on, **zero** required approvals, and the **`handbook` check required** — `erd-check` and `guide-check` over every pull request, added the same day (A-181, issue #3). One setting still deviates from this section and it is not a choice: one collaborator cannot approve their own pull request, so the `1` above deadlocks the repository until a second engineer joins. A-180.

That amendment is itself the argument for **C10 here**. This paragraph asserted *"no required status checks"* and was wrong within the hour, because branch protection is a claim about GitHub settings rather than about files — neither check script can see it, and the residual noted below is not hypothetical. The handbook repository has no weekly drift job.
